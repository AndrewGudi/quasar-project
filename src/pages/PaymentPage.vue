            ...
            section
              ...
              q-form(
                ...
                .flex.column.q-mt-lg
                  ...
                  q-input.q-mt-sm(outlined mask="#### #### #### ####" unmasked-value v-model="formDataSecond.card" placeholder="2548 9841 2570 ****" type="text" :rules="[val => val && val.length > 0 || 'Enter a valid card number', val => /^\\d{16}$/.test(val) || 'Enter a valid card number']")
                    template(v-slot:prepend)
                      q-icon(name="card")
                .flex.column.q-mt-lg
                  .form-title {{'Name on Card'}}
                  q-input.q-mt-sm(outlined v-model="formDataSecond.name" placeholder="Enter name on card" type="text")
                    template(v-slot:prepend)
                      q-icon(name="user")
                .row.justify-between.q-mt-md
                  .flex.column(:style="{width: '45%'}")
                    .flex.column.form-title {{'Expiry Date'}}
                    q-input.q-mt-sm(outlined mask="##/##"
              ...
              q-checkbox.q-mt-md.checkbox-item(v-model="formDataSecond.check" color="black" )
                slot
                  .text
...
import {step, checkBox, onSubmit, userRef, cardRef, formData} from 'src/composables/paymentActions';
